...
/**
 * The Class BlockElectricFurnace.
 */
public class BlockElectricFurnace extends BlockMachine implements ITextureProvider
{
    
    /**
     * Instantiates a new block electric furnace.
     *
     * @param i the i
     * @param j the j
     */
    public BlockElectricFurnace(int i, int j)
    {
        super("Electric Furnace", i, Material.WOOD);
        textureId = j;
        a(i);
        j();
    }
    ...
    public boolean machineActivated(World world, int i, int j, int k, EntityHuman entityhuman)
    {
        if (!world.isStatic)
        {
            entityhuman.openGui(BasicComponents.getInstance(), 2, world, i, j, k);
            return true;
        }
        else
        {
