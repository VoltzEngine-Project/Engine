...
/**
 * An advanced version of pathfinding to find the shortest path between two
 * points. Uses the A* Pathfinding algorithm.
 * 
 * @author Calclavia
 * 
 */
public class PathfinderAStar extends Pathfinder {
	...
	/**
	 * The map of navigated nodes storing the data of which position came from
	 * which in the format of: X came from Y.
	 */
	public HashMap<Vector3, Vector3> navigationMap;

	/**
	 * Score values, used to determine the score for a path to evaluate how
	 * optimal the path is. G-Score is the cost along the best known path while
	 * F-Score is the total cost.
	 */
	...
	public boolean findNodes(Vector3 start) {
		...
		this.fScore.put(start, this.gScore.get(start)
				+ getHeuristicEstimatedCost(start, this.goal));
		...
		while (!this.openSet.isEmpty()) {
			...
			for (Vector3 neighbor : getNeighborNodes(currentNode)) {
				double tentativeGScore = this.gScore.get(currentNode)
				...
				if (!this.openSet.contains(neighbor)
						|| tentativeGScore < this.gScore.get(neighbor)) {
					this.navigationMap.put(neighbor, currentNode);
					this.gScore.put(neighbor, tentativeGScore);
					this.fScore.put(neighbor, gScore.get(neighbor)
							+ getHeuristicEstimatedCost(neighbor, goal));
	 ...
	 * analyzed.
	 */
	public Set<Vector3> reconstructPath(
			HashMap<Vector3, Vector3> nagivationMap, Vector3 current_node) {
		Set<Vector3> path = new HashSet<Vector3>();
		path.add(current_node);

		if (nagivationMap.containsKey(current_node)) {
			path.addAll(reconstructPath(nagivationMap,
					nagivationMap.get(current_node)));
			return path;
		} else {
			return path;
		}
	...
	public Set<Vector3> getNeighborNodes(Vector3 vector) {
		if (this.callBackCheck != null) {
			...
			for (int i = 0; i < 6; i++) {
				neighbors.add(vector.clone().modifyPositionFromSide(
