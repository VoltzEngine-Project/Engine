...
public class ElectricalEventHandler {
	...
	public void onProduce(ElectricityProduceEvent evt) {
		...
		for (ForgeDirection direction : getDirections((TileEntity) evt.tileEntity)) {
			IElectricityNetwork network = getNetworkFromTileEntity(
					VectorHelper.getTileEntityFromSide(evt.world, position,
							direction), direction.getOpposite());

			if (network != null) {
				networks.put(network, direction);
				ElectricityPack provided = evt.tileEntity.provideElectricity(
						direction,
						network.getRequest((TileEntity) evt.tileEntity), true);
		 ...
		 * 
		 * if (request.getWatts() > 0) { List<ElectricityPack> providedPacks =
		 * new ArrayList<ElectricityPack>(); List<TileEntity> tileEntities = new
		 * ArrayList<TileEntity>();
		 * 
		 * for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
		 * Vector3 position = new
		 * Vector3(this).modifyPositionFromSide(direction); TileEntity
		 * tileEntity = position.getTileEntity(this.worldObj);
		 * 
		 * if (tileEntity instanceof IElectrical) {
		 * tileEntities.add(tileEntity); IElectrical electrical = (IElectrical)
		 * tileEntity;
		 * 
		 * if (electrical.canConnect(direction.getOpposite())) { if
		 * (electrical.getProvide(direction.getOpposite()) > 0) {
		 * providedPacks.add
		 * (electrical.provideElectricity(direction.getOpposite(), request,
		 * true)); } } } }
		 * 
		 * ElectricityPack mergedPack = ElectricityPack.merge(providedPacks);
		 * 
		 * if (mergedPack.getWatts() > 0) {
		 * this.getNetwork().produce(mergedPack, tileEntities.toArray(new
		 * TileEntity[0])); } }
		 */
	}
	...
	public static ElectricityPack produceFromMultipleSides(
		...
		return ElectricalEventHandler.produceFromMultipleSides(tileEntity,
	...
	public static ElectricityPack produceFromMultipleSides(
		...
		if (tileEntity != null && approachingDirection != null) {
			final Set<IElectricityNetwork> connectedNetworks = ElectricalEventHandler
	...
	public static Set<IElectricityNetwork> getNetworksFromMultipleSides(
		...
		for (ForgeDirection side : ForgeDirection.VALID_DIRECTIONS) {
			if (approachingDirection.contains(side)) {
				...
				IElectricityNetwork electricityNetwork = ElectricalEventHandler
