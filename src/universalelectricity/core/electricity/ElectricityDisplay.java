...
public class ElectricityDisplay {
	...
	public static enum MeasurementUnit {
		MICRO("Micro", "mi", 0.000001), MILLI("Milli", "m", 0.001), KILO(
				"Kilo", "k", 1000), MEGA("Mega", "M", 1000000);
	...
	public static String getDisplay(double value, ElectricUnit unit,
		...
		if (value <= MeasurementUnit.MILLI.value) {
			return roundDecimals(MeasurementUnit.MICRO.process(value),
					decimalPlaces)
					+ " "
		...
		if (value < 1) {
			return roundDecimals(MeasurementUnit.MILLI.process(value),
					decimalPlaces)
					+ " "
		...
		if (value > MeasurementUnit.MEGA.value) {
			return roundDecimals(MeasurementUnit.MEGA.process(value),
					decimalPlaces)
					+ " "
		...
		if (value > MeasurementUnit.KILO.value) {
			return roundDecimals(MeasurementUnit.KILO.process(value),
					decimalPlaces)
					+ " "
	...
	public static String getDisplaySimple(double value, ElectricUnit unit,
			int decimalPlaces) {
		if (value > 1) {
			if (decimalPlaces < 1) {
