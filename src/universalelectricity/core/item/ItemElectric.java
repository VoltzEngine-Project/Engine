...
public abstract class ItemElectric extends Item implements IItemElectric {
	...
	public void addInformation(ItemStack itemStack, EntityPlayer entityPlayer,
		...
		list.add(color
				+ ElectricityDisplay.getDisplay(joules, ElectricUnit.JOULES)
				+ "/"
				+ ElectricityDisplay.getDisplay(
						this.getMaxElectricityStored(itemStack),
						ElectricUnit.JOULES));
	}
	...
	public float recharge(ItemStack itemStack, float energy, boolean doReceive) {
		float rejectedElectricity = Math.max(
				(this.getElectricityStored(itemStack) + energy)
						- this.getMaxElectricityStored(itemStack), 0);
		float energyToReceive = energy - rejectedElectricity;

		if (doReceive) {
			this.setElectricity(itemStack, this.getElectricityStored(itemStack)
					+ energyToReceive);
		}
	...
	public float discharge(ItemStack itemStack, float energy, boolean doTransfer) {
		float energyToTransfer = Math.min(this.getElectricityStored(itemStack),
				energy);
	...
	public void setElectricity(ItemStack itemStack, float joules) {
		...
		float electricityStored = Math.max(
				Math.min(joules, this.getMaxElectricityStored(itemStack)), 0);
		 ...
		 */
		itemStack
				.setItemDamage((int) (100 - (electricityStored / getMaxElectricityStored(itemStack)) * 100));
	}
	...
	public float getElectricityStored(ItemStack itemStack) {
		...
		float electricityStored = itemStack.getTagCompound().getFloat(
				"electricity");
	...
	public void getSubItems(int par1, CreativeTabs par2CreativeTabs,
		...
		par3List.add(ElectricItemHelper.getWithCharge(chargedItem,
				this.getMaxElectricityStored(chargedItem)));
	}
}
