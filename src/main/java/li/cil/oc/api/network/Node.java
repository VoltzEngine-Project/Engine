 ...
 * <p/>
 * All permissions in a network have a unique address; the network will generate a
 * unique address and assign it to new permissions.
 * <p/>
 * Per default there are two kinds of permissions: tile entities and items.
 * <p/>
 * Items will usually only have permissions when in containers, such as a computer or
 * disk drive. Otherwise you'll have to connect/disconnect them manually as
 * desired.
 * <p/>
 * All other kinds of permissions you may come up with will also have to be
 * handled manually.
 * <p/>
...
public interface Node extends Persistable {
     ...
     * to which permissions. This value should not change over the lifetime of a node.
     * <p/>
     * It furthermore determines what is returned by the <tt>Network</tt>'s
     * <tt>neighbors</tt> and <tt>permissions</tt> functions.
     * <p/>
     * Note that this has no effect on the <em>real</em> reachability of a node;
     * it is only used to filter to which permissions to send connect, disconnect and
     ...
     * it. Therefore permissions should still verify themselves that they want to
     ...
     * permissions, such as cables. In that case they may ignore the address being set.
     */
    String address();
     ...
     * <p/>
     * Note that valid permissions should never return `None` here. When created a node
    ...
    /**
     * Get the list of neighbor permissions, i.e. permissions directly connected to this
     * node.
     * <p/>
     ...
     *
     * @return the list of permissions directly connected to this node.
     */
    Iterable<Node> neighbors();

    /**
     * Get the list of permissions reachable from this node, based on their
     * {@link #reachability()}.
     * <p/>
     * This is a shortcut for <tt>node.network.permissions(node)</tt>.
     * <p/>
     * If this node is not in a network, i.e. <tt>network</tt> is <tt>null</tt>,
     * this returns an empty list.
     *
     * @return the list of permissions reachable from this node.
     */
    ...
    /**
     * Send a message to all permissions reachable from this node.
     * <p/>
    ...
    /**
     * Send a message to all permissions visible from this node.
     * <p/>
