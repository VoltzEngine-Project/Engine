	...
	public boolean called(EntityPlayer player, ITerminal terminal, String[] args)
	{
		if (args[0].equalsIgnoreCase("users") && args.length > 1 && args[1] != null)
		{
			...
			if (args[1].equalsIgnoreCase("List"))
			{
				...
				for (int i = 0; i < terminal.getUsers().size(); i++)
				{
					terminal.addToConsole(" " + i + ") " + terminal.getUsers().get(i).getName());
				}
				return true;
			}
			if (args[1].equalsIgnoreCase("remove") && args.length > 2)
			{
				if (args[2] != null)
				{
					if (terminal.setUserAccess(args[2], null, false))
					{
			...
			if (args[1].equalsIgnoreCase("add") && args.length > 2)
			{
				if (args[2] != null && terminal.getGroup(args[2]) != null)
				{
					if (args.length > 3)
					{
						...
						else if (terminal.setUserAccess(args[3], terminal.getGroup(args[2]), true))
						{
