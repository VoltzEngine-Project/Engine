...
public class ClientRegistryProxy extends CommonRegistryProxy
{
    ...
    public void registerTileEntity(String name, Class<? extends TileEntity> tileClass)
    {
        ...
        {
            //From DarkGuardsman: Ignore these types of exceptions as this just means the block has no renderer for the tile given
        }
        catch (Exception e)
        {
            Calclavia.LOGGER.severe("[ClientContentRegistry]Failed to register TileEntity renderer for " + name);
            e.printStackTrace();
