...
public class Components
{
	...
	public static void init()
	{
		...
		Calclavia.LOGGER.fine("Loaded: " + TranslationHelper.loadLanguages(CalclaviaCoreLoader.LANGUAGE_PATH, CalclaviaCoreLoader.LANGUAGES_SUPPORTED) + " Languages.");

		CalclaviaCoreLoader.metadata.modId = CalclaviaCoreLoader.ID;
		CalclaviaCoreLoader.metadata.name = CalclaviaCoreLoader.NAME;
		CalclaviaCoreLoader.metadata.description = "Calclavia Core is an FML Plugin to provide core functionalities in order to run Calclavia's mods.";
		CalclaviaCoreLoader.metadata.url = "http://universalelectricity.com/calclavia-core";
		CalclaviaCoreLoader.metadata.version = CalclaviaCoreLoader.VERSION + CalclaviaCoreLoader.BUILD_VERSION;
		CalclaviaCoreLoader.metadata.authorList = Arrays.asList(new String[] { "Calclavia" });
		CalclaviaCoreLoader.metadata.autogenerated = false;
	}
	...
	public static Item requireItem(String fieldName, int id)
	{
		try
		{
			...
			Field field = ReflectionHelper.findField(Components.class, fieldName);
			Item f = (Item) field.get(null);

			// Grabs the default ID.
			Field idField = ReflectionHelper.findField(Components.class, "id" + Character.toUpperCase(fieldName.charAt(0)) + fieldName.substring(1));
		...
		try
		{
			...
			if (f == null)
			{
				if (name.contains("ore"))
				{
					...
					Field generationField = ReflectionHelper.findField(Components.class, "generation" + Character.toUpperCase(name.charAt(0)) + name.substring(1));
