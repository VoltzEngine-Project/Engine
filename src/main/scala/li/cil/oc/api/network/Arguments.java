 ...
 * <p/>
 * It allows checking for the presence of arguments in a uniform manner, taking care of proper type
 * checking based on what can be passed along by Lua.
 * <p/>
 * Note that integer values fetched this way are actually double values that have been truncated. So
 * if a Lua program passes <tt>1.9</tt> and you do a <tt>checkInteger</tt> you'll get a <tt>1</tt>.
 * <p/>
 * The indexes passed to the various functions start at zero, i.e. to get the first argument you
 * would use <tt>checkAny(0)</tt>. This is worth mentioning because Lua starts its indexes at one. */
public interface Arguments extends Iterable<Object>
{
     ...
     * <p/>
     * The returned object will be one of the following, based on the conversion performed
     * internally:
     * <ul>
     ...
     * @throws IllegalArgumentException if there is no argument at that index, or if the argument is
     * not a boolean. */
     ...
     * <p/>
     * This will return false if there is <em>no</em> argument at the specified index, i.e. if there
