...
/** TileEntities can implement the {@link li.cil.oc.api.network.SidedEnvironment} interface to allow
 * them to interact with the component network, by providing a separate
 * {@link li.cil.oc.api.network.Node} for each block face, and connecting it to said network. This
 * allows more control over connectivity than the simple {@link li.cil.oc.api.network.Environment}.
 * <p/>
 * Nodes in such a network can communicate with each other, or just use the network as an index
 * structure to find other nodes connected to them. */
