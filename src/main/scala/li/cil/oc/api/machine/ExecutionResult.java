...
{
    /** Indicates the machine may sleep for the specified number of ticks. This is merely considered
     * a suggestion. If signals are in the queue or are pushed to the queue while sleeping, the
     * sleep will be interrupted and {@link Architecture#runThreaded(boolean)} will be called so
     * that the next signal is pushed. */
    ...
    /** Indicates that a synchronized call should be performed. The architecture is expected to be in
     * a state that allows the next call to be to {@link Architecture#runSynchronized()} instead of
     * {@link Architecture#runThreaded(boolean)}. This is used to perform calls from the server's
     * main thread, to avoid threading issues when interacting with other objects in the world. */
