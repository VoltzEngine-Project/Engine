...
public interface Handle
{
    ...
    /** Closes the handle.
     * <p/>
     * For example, if there is an underlying stream, this should close that stream. Any future
     * calls to {@link #read} or {@link #write} should throw an <tt>IOException</tt> after this
     * function was called. */
    void close();

    /** Tries to read as much data from the file as fits into the specified array.
     * <p/>
     * For files opened in write or append mode this should always throw an exception.
     * 
     * @param into the buffer to read the data into.
     * @return the number of bytes read; -1 if there are no more bytes (EOF).
     * @throws IOException if the file was opened in writing mode or an I/O error occurred or the
     ...
     * @throws IOException if the file was opened in read-only mode, or another I/O error occurred
     * (no more space, for example), or the file was already closed. */
