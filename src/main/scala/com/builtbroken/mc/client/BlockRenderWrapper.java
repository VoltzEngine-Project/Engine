...
/**
 * Used to fake data for rendering blocks
 *
...
public class BlockRenderWrapper extends Block
{
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z)
    {
        if (otherErrors.get(realBlock) == null || otherErrors.get(realBlock).size() < suppressErrorLimit)
        {
            ...
            {
                Engine.logger().error("BlockWrapper#colorMultiplier" + world + ", " + x + "," + y + "," + z
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int side)
    {
        if (canRenderSide(side))
        {
            if (otherErrors.get(realBlock) == null || otherErrors.get(realBlock).size() < suppressErrorLimit)
            {
                ...
                {
                    Engine.logger().error("BlockWrapper#shouldSideBeRendered" + world + ", " + x + "," + y + "," + z + ", " + side
    ...
    public int getMixedBrightnessForBlock(IBlockAccess world, int x, int y, int z)
    {
        if (otherErrors.get(realBlock) == null || otherErrors.get(realBlock).size() < suppressErrorLimit)
        {
            ...
            {
                Engine.logger().error("BlockWrapper#getMixedBrightnessForBlock(" + world + ", " + x + "," + y + "," + z
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side)
    {
        if (textureErrors.get(realBlock) == null || textureErrors.get(realBlock).size() < suppressErrorLimit)
        {
            ...
            {
                Engine.logger().error("BlockWrapper#getIcon(" + world + ", " + x + "," + y + "," + z + "," + side
    ...
    public IIcon getIcon(int side, int meta)
    {
        if (textureErrors.get(realBlock) == null || textureErrors.get(realBlock).size() < suppressErrorLimit)
        {
            ...
            {
                Engine.logger().error("BlockWrapper#getIcon(" + side + "," + meta
                        + ") - Unexpected error while getting icon for side on block '" + InventoryUtility.getRegistryName(realBlock) + "'", e);
                addTextureError(e);
            }
        }
