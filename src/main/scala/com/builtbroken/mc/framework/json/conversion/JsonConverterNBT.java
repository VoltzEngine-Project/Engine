    ...
    public static JsonElement toJson(NBTTagCompound nbt)
    {
        if (nbt != null && !nbt.hasNoTags())
        {
            ...
            for (String key : nbt.getKeySet())
            {
                ...
                {
                    object.add(key + ":byte", new JsonPrimitive(((NBTTagByte) tag).getByte()));
                }
                else if (tag instanceof NBTTagShort)
                {
                    object.add(key + ":short", new JsonPrimitive(((NBTTagShort) tag).getShort()));
                }
                else if (tag instanceof NBTTagInt)
                {
                    object.add(key + ":int", new JsonPrimitive(((NBTTagInt) tag).getDouble()));
                }
                else if (tag instanceof NBTTagLong)
                {
                    object.add(key + ":long", new JsonPrimitive(((NBTTagLong) tag).getLong()));
                }
                else if (tag instanceof NBTTagFloat)
                {
                    object.add(key + ":float", new JsonPrimitive(((NBTTagFloat) tag).getFloat()));
                }
                else if (tag instanceof NBTTagDouble)
                {
                    object.add(key + ":double", new JsonPrimitive(((NBTTagDouble) tag).getDouble()));
                }
                ...
                {
                    object.add(key, new JsonPrimitive(((NBTTagString) tag).getString()));
                }
