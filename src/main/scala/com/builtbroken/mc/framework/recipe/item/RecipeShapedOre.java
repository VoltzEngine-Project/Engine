...
public class RecipeShapedOre extends ShapedOreRecipe
{
    ...
    public RecipeShapedOre(Block result, Object... recipe)
    {
        this(new ItemStack(result), recipe);
    }

    public RecipeShapedOre(Item result, Object... recipe)
    {
        this(new ItemStack(result), recipe);
    }
        ...
        for (int x = 0; x < MAX_CRAFT_GRID_WIDTH; x++)
        {
            for (int y = 0; y < MAX_CRAFT_GRID_HEIGHT; y++)
            {
                ...
                if (!target.apply(inv.getStackInRowAndColumn(x, y)))
                {
                    ...
                    {
                        if (itemMatches(itemstack, inv.getStackInRowAndColumn(x, y), false))
                        {
                            found = true;
                            break;
                        }
