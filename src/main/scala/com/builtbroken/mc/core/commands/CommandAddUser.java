 ...
 */
public class CommandAddUser extends ModularCommand
{
            ...
            if (username != null && !username.isEmpty())
            {
                if (args.length > 1)
                {
                    ...
                    if (command.equalsIgnoreCase("to"))
                    {
                        ...
                        {
                            sender.addChatMessage(new ChatComponentText("Need to know what to add the user to"));
                            return true;
                        }
                    }
    ...
    public void getHelpOutput(ICommandSender sender, List<String> items)
    {
        ...
        for (AbstractCommand command : subCommands)
        {
            ...
            {
                items.add(command.getCommandName() + " [username] to " + s);
            }
        }
    }
}
