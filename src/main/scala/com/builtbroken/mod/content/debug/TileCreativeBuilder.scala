package com.builtbroken.mod.content.debug
...
import com.builtbroken.lib.network.discriminator.{PacketTile, PacketType}
import com.builtbroken.lib.network.handle.{TPacketReceiver, TPacketSender}
import com.builtbroken.lib.prefab.tile.TileAdvanced
import com.builtbroken.lib.world.schematic.SchematicRegistry
...
class TileCreativeBuilder extends TileAdvanced(Material.iron) with TRotatable with TPacketReceiver with TPacketSender
{
  ...
  {
    player.openGui(BBL.instance, -1, world, x.toInt, y.toInt, z.toInt)
    return true
  }
