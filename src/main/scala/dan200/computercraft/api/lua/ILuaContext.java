...
{
    /** Wait for an event to occur on the computercraft, suspending the thread until it arises. This
     ...
     * attempt to common this exception, unless you wish to prevent termination, which is not
     * recommended.
     * @throws InterruptedException If the user shuts down or reboots the computercraft while
     * pullEvent() is waiting for an event, InterruptedException will be thrown. This exception must
     * not be caught or intercepted, or the computercraft will leak memory and end up in a broken
     * state. */
     ...
     * must not be caught or intercepted, or the computercraft will leak memory and end up in a
     * broken state.
     ...
     * @throws InterruptedException If the user shuts down or reboots the computercraft the
     * coroutine is suspended, InterruptedException will be thrown. This exception must not be
     * caught or intercepted, or the computercraft will leak memory and end up in a broken state.
     * @see #pullEvent(String) */
