...
object UELoader extends IFMLLoadingPlugin with IFMLCallHook
{
  ...
  def preInit(evt: FMLPreInitializationEvent)
  {
    ...
    config.load
    CompatibilityType.THERMAL_EXPANSION.reciprocal_ratio = config.get("Compatibility", "Thermal Expansion Conversion Ratio", CompatibilityType.THERMAL_EXPANSION.reciprocal_ratio).getDouble(CompatibilityType.THERMAL_EXPANSION.reciprocal_ratio)
    CompatibilityType.BUILDCRAFT.reciprocal_ratio = config.get("Compatibility", "BuildCraft Conversion Ratio", CompatibilityType.BUILDCRAFT.reciprocal_ratio).getDouble(CompatibilityType.BUILDCRAFT.reciprocal_ratio)
    ...
    for (compatibility <- CompatibilityType.getModules)
    {
      compatibility.isModuleEnabled = config.get("Compatibility", "Load " + compatibility.moduleName + " Module", true).getBoolean(true)
      if (compatibility.isModuleEnabled)
      {
        try
        {
          CompatibilityModule.register((compatibility.moduleClass).newInstance.asInstanceOf[CompatibilityModule])
        }
        catch
          {
            case e: Exception =>
  ...
  /** Inject coremod data into this coremod This data includes: "mcLocation" : the location of the
    ...
    * coremod loaded from, */
  def injectData(data: Map[String, AnyRef])
  {
  }
  ...
  def getAccessTransformerClass: String =
  {
    return null
  }
}