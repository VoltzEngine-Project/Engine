...
class Vector3 (var x : Double, var y : Double, var z : Double) extends Cloneable with TraitVector[Vector3]
{
    ...
    {
        new Vector3(x + amount, y + amount, z + amount)
    }
    ...
    {
        Math.acos((this $ other) / (magnitude * other.magnitude))
    }
    ...
    def writeToNBT (world : IBlockAccess, nbt : NBTTagCompound) : NBTTagCompound =
    {
        this.getTileEntity(world).writeToNBT(nbt)
        return nbt
    }
