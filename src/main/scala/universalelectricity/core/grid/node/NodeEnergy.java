 ...
 * <p/>
 * TODO: Energy storage should later be handled by the TileEntity itself. Most blocks will be components of a circuit and do not store energy but instead consume it.
 *
...
public class NodeEnergy extends NodeConnector implements IEnergyNode, IUpdate, ISave
{
	...
	public double addEnergy(ForgeDirection from, double energy, boolean doAdd)
	{
		if (canConnect(from))
		{
			return buffer.receiveEnergy(energy, doAdd);
		}
		return 0;
	}
	...
	{
		if (canConnect(from) && canShareEnergy())
		{
			return buffer.extractEnergy(energy, doRemove);
		}
		return 0;
	}
