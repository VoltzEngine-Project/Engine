...
/**
 * A synthetic-block based multiblock structure uses fake invisible blocks that are automatically placed
 * to create block bounds.
 *
 * @author Calclavia
 */
public class BlockSyntheticPart extends BlockContainer
{
	public String textureName = null;
	@Deprecated
	public PacketTile packetType;

	public BlockSyntheticPart()
	{
		super(Material.iron);
	...
	@Override
	public BlockSyntheticPart setBlockTextureName(String name)
	{
		this.textureName = name;
		return this;
	}

	public void createStructure(IMultiBlock tile)
	{
	...
	public void destroyStructure(IMultiBlock tile)
	{
		TileEntity tileEntity = (TileEntity) tile;
		Vector3[] positions = tile.getMultiBlockVectors();
	...
	public IIcon getIcon(IBlockAccess blockAccess, int x, int y, int z, int par5)
	{
		try
		{
			Vector3 main = ((TileSyntheticPart) blockAccess.getTileEntity(x, y, z)).getMainBlock();
	...
	public void breakBlock(World world, int x, int y, int z, Block par5, int par6)
	{
		...
		if (tileEntity instanceof TileSyntheticPart)
		{
			((TileSyntheticPart) tileEntity).onBlockRemoval(this);
		}
	...
	{
		TileSyntheticPart tileEntity = (TileSyntheticPart) par1World.getTileEntity(x, y, z);
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World par1World, int x, int y, int z)
	{
		...
		Vector3 mainBlockPosition = ((TileSyntheticPart) tileEntity).getMainBlock();
