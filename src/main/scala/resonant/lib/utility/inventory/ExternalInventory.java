...
public class ExternalInventory implements IExternalInventory
{
    ...
    {
        return host instanceof TileEntity && tile().getWorldObj().getTileEntity(tile().xCoord, tile().yCoord, tile().zCoord) != tile() ? false : par1EntityPlayer.getDistanceSq(tile().xCoord + 0.5D, tile().yCoord + 0.5D, tile().zCoord + 0.5D) <= 64.0D;
