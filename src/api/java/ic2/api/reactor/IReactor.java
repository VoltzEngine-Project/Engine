...
public interface IReactor
{
    ...
    /** Set the reactor's stored maxHeat variable. Used by plating to increase the reactors MaxHeat
     * capacity. Needs to be called during each cycle process. */
    public void setMaxHeat(int newMaxHeat);

    /** add Heat to a EmitHeat Buffer for use in Reactor operation.. need to be use for all
     * Componetents with self-cooling no more magic heat disappear */

    public void addEmitHeat(int heat);

    /** Get's the reactor's HEM (Heat Effect Modifier) Basic value is 1.0F. Reducing the value causes
     * a weakening/reduction of the heat-based sideeffects of reactors (F.e. water evaporation,
     * melting, damaging entitys, etc)
     * 
     * @return HEM */
    public float getHeatEffectModifier();

    /** Set's the reactor's HEM Needs to be called during each cycle process. */
