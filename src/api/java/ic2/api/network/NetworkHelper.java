...
public final class NetworkHelper
{
     ...
     * 
     * The updater will query the field's value during the next update, updates happen usually every
     * 2 ticks. If low latency is important use initiateTileEntityEvent instead.
     * 
     * IC2's network updates have to get triggered every time, it doesn't continuously poll/send the
     * field value. Just call updateTileEntityField after every change to a field which needs
     * network synchronization.
     * 
     * The following field data types are currently supported: - int, int[], short, short[], byte,
     * byte[], long, long[] - float, float[], double, double[] - boolean, boolean[] - String,
     * String[] - ItemStack - NBTBase (includes NBTTagCompound) - Block, Item, Achievement, Potion,
     * Enchantment - ChunkCoordinates, ChunkCoordIntPair - TileEntity (does not sync the actual tile
     * entity, instead looks up the tile entity by its position in the client world) - World (does
     * not sync the actual world, instead looks up the world by its dimension ID)
     * 
     ...
     * 
     * The implementation is currently limited to TileEntitys as data providers. The tile entity has
     * to be fully initialized when executing this method (i.e. valid worldObj+coords).
     * 
