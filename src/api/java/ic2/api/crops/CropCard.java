...
public abstract class CropCard
{
    ...
    /** Tier of the plant. Ranges from 1 to 16, 0 is Weed. Valuable and powerful crops have higher
     * tiers, useless and weak ones have lower tiers.
     * 
     * @return Tier */
    public abstract int tier();

    /** Describe the plant through a set of stats, influencing breeding. Plants sharing stats and
     * attributes will tend to cross-breed more often.
     * 
     * Stats: - 0: Chemistry (Industrial uses based on chemical plant components) - 1: Consumable
     * (Food, potion ingredients, stuff meant to be eaten or similarly used) - 2: Defensive (Plants
     * with defense capabilities (damaging, explosive, chemical) or special abilities in general) -
     * 3: Colorful (How colorful/aesthetically/beautiful is the plant, like dye-plants or plants
     * without actual effects) - 4: Weed (Is this plant weed-like and rather
     * unwanted/quick-spreading? Rare super-breed plants should have low values here)
     * 
     * @param n index of the requested stat
     * @return The requested value of the stats */
    public abstract int stat(int n);

    /** Additional attributes of the plant, also influencing breeding. Plants sharing stats and
     * attributes will tend to cross-breed more often.
     * 
    ...
    /** Get the crop's spritesheet. Per default crops_0.png of ic2-sprites
     * 
    ...
    /** Amount of growth points needed to increase the plant's size. Default is 200 * tier. */
     ...
     * 
     * Consider: - Humidity, nutrients and air quality - Current size - Light level - Special biomes
     * or conditions, accessible through crop.worldObj
     * 
     * This method will be called upon empty upgraded crops to check whether a neighboring plant can
     * cross onto it! Don't check if the size is greater than 0 and if the ID is real.
     * 
     * @param crop reference to ICropTile
     * @return Whether the crop can grow */
    public abstract boolean canGrow(ICropTile crop);

    /** Calculate the influence for the plant to grow based on humidity, nutrients and air. Normal
     * behavior is rating the three stats "normal", with each of them ranging from 0-30. Basic rule:
     * Assume everything returns 10. All together must equal 30. Add the factors to your likings,
     * for example (humidity*0.7)+(nutrients*0.9)+(air*1.4)
     * 
    ...
    /** Get the size of the plant after harvesting. Default is 1.
     * 
     * @param crop reference to ICropTile
    ...
    /** Base chance for dropping seeds when the plant is picked. Default is 0.5*0.8^tier with a
     * bigger chance for sizes greater than 2 and absolutely no chance for size 0.
     * 
     * @param crop reference to ICropTile
     ...
     * @return Whether trampling calculation should happen, return false if the plant is no longer
     * valid. */
    ...
    /** Called every time the crop ticks. Should be called every 256 ticks or around 13 seconds.
     * 
     * @param crop reference to ICropTile */
    ...
    /** Check whether this plant spreads weed to surrounding tiles. Default is true if the plant has
     * a high growth stat (or is weeds) and size greater or equal than 2.
     * 
     * @param crop reference to ICropTile
