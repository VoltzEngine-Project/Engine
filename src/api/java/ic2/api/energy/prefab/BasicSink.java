 ...
 * 
 * The constraints set by BasicSink like the strict tank-like energy buffering should provide a more
 * easy to use and stable interface than using IEnergySink directly while aiming for optimal
 * performance.
 * 
 * Using BasicSink involves the following steps: - create a BasicSink instance in your TileEntity,
 * typically in a field - forward invalidate, onChunkUnload, readFromNBT, writeToNBT and
 * updateEntity to the BasicSink instance. If you have other means of determining when the tile
 * entity is fully loaded, notify onLoaded that way instead of forwarding updateEntity. - call
 * useEnergy whenever appropriate. canUseEnergy determines if enough energy is available without
 * consuming the energy. - optionally use getEnergyStored to display the output buffer charge level
 * - optionally use setEnergyStored to sync the stored energy to the client (e.g. in the Container)
 * 
 * Example implementation code:
 * 
 * @code{.java public class SomeTileEntity extends TileEntity { // new basic energy sink, 1000 EU
 * buffer, tier 1 (32 EU/t, LV) private BasicSink ic2EnergySink = new BasicSink(this, 1000, 1);
 * 
 * @Override public void invalidate() { ic2EnergySink.invalidate(); // notify the energy sink ...
 * super.invalidate(); // this is important for mc! }
 * 
 * @Override public void onChunkUnload() { ic2EnergySink.onChunkUnload(); // notify the energy sink
 * ... }
 * 
 ...
 * 
 * @Override public void updateEntity() { ic2EnergySink.updateEntity(); // notify the energy sink
 * ... if (ic2EnergySink.useEnergy(5)) { // use 5 eu from the sink's buffer this tick ... // do
...
public class BasicSink extends TileEntity implements IEnergySink
{
    ...
    /** Notification that the base TileEntity finished loaded, for advanced uses. Either updateEntity
     * or onLoaded have to be used. */
    ...
    /** Forward for the base TileEntity's invalidate(), used for destroying the energy net link. Both
     * invalidate and onChunkUnload have to be used. */
    @Override
     ...
     * 
     * This is intended for server -> client synchronization, e.g. to display the stored energy in a
     * GUI through getEnergyStored().
     * 
     * @param amount */
