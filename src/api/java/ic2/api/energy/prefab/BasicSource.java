 ...
 * more easy to use and stable interface than using IEnergySource directly while aiming for optimal
 * performance.
 * 
 * Using BasicSource involves the following steps: - create a BasicSource instance in your
 * TileEntity, typically in a field - forward invalidate, onChunkUnload, readFromNBT, writeToNBT and
 * updateEntity to the BasicSource instance. If you have other means of determining when the tile
 * entity is fully loaded, notify onLoaded that way instead of forwarding updateEntity. - call
 * addEnergy whenever appropriate, using getFreeCapacity may determine if e.g. the generator should
 * run - optionally use getEnergyStored to display the output buffer charge level - optionally use
 * setEnergyStored to sync the stored energy to the client (e.g. in the Container)
 * 
 * Example implementation code:
 * 
 * @code{.java public class SomeTileEntity extends TileEntity { // new basic energy source, 1000 EU
 * buffer, tier 1 (32 EU/t, LV) private BasicSource ic2EnergySource = new BasicSource(this, 1000,
 * 1);
 * 
 * @Override public void invalidate() { ic2EnergySource.invalidate(); // notify the energy source
 * ... super.invalidate(); // this is important for mc! }
 * 
 * @Override public void onChunkUnload() { ic2EnergySource.onChunkUnload(); // notify the energy
 * source ... }
 * 
 ...
 * 
 * @Override public void updateEntity() { ic2EnergySource.updateEntity(); // notify the energy
 * source ... ic2EnergySource.addEnergy(5); // add 5 eu to the source's buffer this tick ... }
 * 
 * ... }
 * @endcode */
public class BasicSource extends TileEntity implements IEnergySource
{
    ...
    /** Notification that the base TileEntity finished loading, for advanced uses. Either
     * updateEntity or onLoaded have to be used. */
