...
public class RectangleTest extends TestCase
{
    ...
    public void testArea()
    {
        ...
        rect = new Rectangle(new Pos2D(-2, -2), new Pos2D(2, 2));
        assertEquals("Expected an exact match for area check three", 16.0, rect.getArea());

        rect = new Rectangle(new Pos2D(10, 20), new Pos2D(-20, -10));
    ...
    {
        Rectangle rect = new Rectangle(new Pos2D(0, 0), new Pos2D(2, 2));

        List<Pos2D> points_inside = new LinkedList();
        points_inside.add(rect.cornerA());
        points_inside.add(rect.cornerB());
        points_inside.add(rect.cornerC());
        points_inside.add(rect.cornerD());
        points_inside.add(new Pos2D(1, 1));

        List<Pos2D> points_outside = new LinkedList();
        points_outside.add(new Pos2D(-1, -1));
        points_outside.add(new Pos2D(0, -1));
        points_outside.add(new Pos2D(-1, 0));
        points_outside.add(new Pos2D(3, 0));
        points_outside.add(new Pos2D(0, 3));
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            List<Pos2D> l = inside ? points_inside : points_outside;
